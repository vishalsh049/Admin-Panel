  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow mb-4 dark:bg-slate-900 dark:shadow-none dark:border dark:border-slate-800">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-lg font-semibold dark:text-slate-100">Payment</h3>
      ...
      {message && (
        <div
          className={`mb-3 px-4 py-2.5 rounded-lg text-sm ${
            message.type === "success" ? "bg-green-50 text-green-700 dark:bg-emerald-500/10 dark:text-emerald-300" : "bg-red-50 text-red-700 dark:bg-rose-500/10 dark:text-rose-300"
          }`}
        >
          {message.text}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-y-3 gap-x-12 text-sm dark:text-slate-300">
        <div>
          <p className="font-semibold dark:text-slate-100">Method:</p>
          <p className="capitalize">{isOnline ? "Online (Razorpay)" : order.paymentMethod || "cod"}</p>
        </div>
        <div>
          <p className="font-semibold dark:text-slate-100">Paid:</p>
          <p>{order.isPaid ? `Yes${order.paidAt ? ` — ${new Date(order.paidAt).toLocaleString()}` : ""}` : "No"}</p>
        </div>
        {isOnline && (
          <>
            <div>
              <p className="font-semibold dark:text-slate-100">Razorpay Order ID:</p>
              <p className="font-mono text-xs">{order.razorpayOrderId || "—"}</p>
            </div>
            <div>
              <p className="font-semibold dark:text-slate-100">Razorpay Payment ID:</p>
              <p className="font-mono text-xs">{order.razorpayPaymentId || "—"}</p>
            </div>
            {order.refundId && (
              <>
                <div>
                  <p className="font-semibold dark:text-slate-100">Refund ID:</p>
                  <p className="font-mono text-xs">{order.refundId}</p>
                </div>
                <div>
                  <p className="font-semibold dark:text-slate-100">Refund Amount:</p>
      ...
      {isOnline && (
        <div className="flex flex-wrap items-center gap-3 mt-4 pt-4 border-t dark:border-slate-800">
          <button
            onClick={handleSync}
            disabled={busy}
            className="px-4 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-medium disabled:opacity-50 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-200"
          >
          ...
          {canRefund && (
            <div className="flex items-center gap-2">
              <input
                ...
                className="border border-slate-200 rounded-lg px-3 py-2 text-sm w-40 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"
              />
              <button
